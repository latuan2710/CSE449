...
def create_train_data_area():
    ...
    day_to_number = {'mon': 2, 'tue': 3, 'wed': 4, 'thu': 5, 'fri': 6, 'sat': 7, 'sun': 8}

    x = data.loc[:, ['X', 'Y', 'month', 'day', 'FFMC', 'DMC', 'DC', 'ISI', 'temp', 'RH', 'wind', 'rain']].values

    for row in x:
        row[2] = month_to_number[row[2]]
        row[3] = day_to_number[row[3]]

    return x, data.loc[:, 'area'].values
...
print(predict(7, 5, 3, 6, 86.2, 26.2, 94.3, 5.1, 8.2, 51, 6.7, 0,
